...
def get_training_set(data_dir, lr, hr_depth, hr_edge, hr_rgb, upscale_factor, patch_size, data_augmentation):
    lr_dir = join(data_dir, lr)
    hr_dir = join(data_dir, hr_depth)
    edge_dir = join(data_dir, hr_edge)
    rgb_dir = join(data_dir, hr_rgb)
    #crop_size = calculate_valid_crop_size(crop_size, upscale_factor)

    return DatasetFromFolder(hr_dir, lr_dir,edge_dir,rgb_dir, patch_size, upscale_factor, lr, data_augmentation,
